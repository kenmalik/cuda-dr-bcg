 ...
 *
 * Solves the block linear system AX = B using the DR-BCG algorithm, taking
 * vectors and allocating device memory as required.
 *
 ...
 * @return Tuple containing the solution X (as a std::vector<float>) and the
 * number of iterations performed
 */
std::tuple<std::vector<float>, int>
dr_bcg::dr_bcg(const std::vector<float> &A, const std::vector<float> &X,
               const std::vector<float> &B, const int n, const int s,
               const float tolerance, const int max_iterations) {
    ...
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void **>(&d_X), sizeof(float) * n * s));
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void **>(&d_B), sizeof(float) * n * s));

    CUDA_CHECK(cudaMemcpy(d_A, A.data(), sizeof(float) * n * n,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_X, X.data(), sizeof(float) * n * s,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_B, B.data(), sizeof(float) * n * s,
                          cudaMemcpyHostToDevice));

    CUSOLVER_CHECK(dr_bcg(cusolverH, cusolverParams, cublasH, n, s, d_A, d_X,
                          d_B, tolerance, max_iterations, &iterations));

    CUDA_CHECK(cudaMemcpy(X_final.data(), d_X, sizeof(float) * n * s,
...
void dr_bcg::quadratic_form(cublasHandle_t &cublasH,
    ...
    CUSPARSE_CHECK(cusparseSpMM_bufferSize(
        cusparseH, X_transpose, A_transpose, &alpha, A, X, &beta, work_descr,
    ...
    CUSPARSE_CHECK(cusparseSpMM(cusparseH, X_transpose, A_transpose, &alpha, A,
